                 ...
                 double* jacobianMatrix)
{
  ...
  if (!(fvec = (double *) malloc(m * sizeof(double))) ||
      ...
      !(fjac = jacobianMatrix != 0 ? jacobianMatrix : (double *) malloc(n * m * sizeof(double))) ||
  ...
  lm_lmdif(m, n, par, fvec, control->ftol, control->xtol, control->gtol,
           ...
           evaluate, printout, jacobian, data);
              ...
              void *data)
{
  ...
  /* Modified by TM 20110418 */
  /* static double *temp_fjac = 0; */
  ...
  *info = 0;
  ...
  // If an external function is provided for calculating the Jacobian,
  // reserve space for the result.
  /* Modified by TM 20110418 */
  /*if (jacobian != 0)
    temp_fjac = (double*)malloc(n * m * sizeof(double));
  */
  ...
  do {
    ...
    if (jacobian == 0) {
      ...
      (*jacobian)(x, m, fjac, data);
      /* Modified by TM 20110418 */
      /* (*jacobian)(x, m, temp_fjac, data); */
      /* Transpose and negate the Jacobian. This algoritm is a Fortran
         port, which shows up in the arrangement of the matrix.
         Furthermore, the Jacobian is used for steepest descent, not
         steepest ascent.
      */
      /*
      for (i = 0; i < m; i++) {
        for (j = 0; j < n; j++)
          fjac[j * m + i] = -temp_fjac[i * n + j];
      }
      */
    }
#if BUG>2
